package me.TechsCode.SpigotAPI.server.routs.data;
...
public class Reviews implements HttpHandler {
    public void handle(HttpExchange t) throws IOException {
        ...
        if(params.get("token") !=null){
            ...
            if(HttpRouter.isTokenValid(token)){
                ...
                ReviewsList reviewsList = HttpRouter.getDataManager().getDataset().getReviews();
                reviewsList.stream().map(Review::getState).forEach(reviews::add);

                obj.put("data", reviews);
